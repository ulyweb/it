<# ============================================
   IT HealthCheck Dashboard V7 (LOCAL)
   Architecture: Direct-Run SPA
   Run As: Administrator on Target Device
============================================ #>
<# ============================================
...
Write-Host "`nSession ended." -ForegroundColor Yellow
if (-not (Test-Path "C:\Temp")) { New-Item -ItemType Directory -Path "C:\Temp" -Force | Out-Null }
try {
    $newLogs = Get-ChildItem -Path "C:\Temp\$env:COMPUTERNAME`_capture_log_*.csv","C:\Temp\$env:COMPUTERNAME`_*.html" -ErrorAction SilentlyContinue | Where-Object { $_.LastWriteTime -gt (Get-Date).AddHours(-2) }
    if ($newLogs) { Write-Host "[SUCCESS] Reports saved to: C:\Temp\" -ForegroundColor Green; Invoke-Item "C:\Temp\\" }
    else { Write-Host "[INFO] No logs generated." -ForegroundColor DarkGray }
} catch { Write-Host "[INFO] Log check completed." -ForegroundColor DarkGray }
