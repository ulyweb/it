<#
================================================================================
 WIFI CONNECTIVITY MONITOR v3.0  --  Windows
 

 Companion to WiFi-Connectivity-Monitor-v3 for macOS. Both builds share the
 ...
 USAGE
   ...
   powershell.exe -ExecutionPolicy Bypass -File .\WiFi-Connectivity-Monitor-v3.ps1 -Samples 180 -Location "Wired Ethernet Test"
================================================================================
#>
...
if ([string]::IsNullOrWhiteSpace($modelID)) { $modelID = "$($cs.SystemSKUNumber)".Trim() }
...
$wlanState = Get-NetshValue $wlanIfaceRaw "State"
...
$netshIface = $wlanIfaceRaw
$profiles   = $wlanProfileRaw
...
[void]$sb.Append("ifc:$(ConvertTo-JsString $netshIface),")
[void]$sb.Append("scutil:$(ConvertTo-JsString $ipconfigRaw),")
[void]$sb.Append("pref:$(ConvertTo-JsString $profiles),")
...
.p-body{padding:12px;max-height:1800px;overflow:auto;transition:max-height .3s ease,padding .3s ease}
...
<div class='brand'><span class='bolt'>&#128225;</span><span class='b1'>WIFI</span><span class='b2'>CONNECTIVITY MONITOR</span><span class='ver'>v3.0</span></div>
<span class='live'><span class='dot'></span>CAPTURED</span>
<span class='statuspill sp-ok' id='platPill'>WINDOWS</span>
...
<footer>
<div><b class='c-cyan'>WIFI CONNECTIVITY MONITOR v3.0</b> &middot; IT Support &middot; read-only capture, no network settings were changed.</div>
...
<div>Signal, channel and neighbour data are read from netsh wlan. Windows reports signal as a link-quality percentage; RSSI is derived from it using the standard quality-to-dBm conversion. Windows does not expose a noise floor, so SNR is not shown &mdash; run the macOS capture on a Mac at the same desk when SNR is needed.</div>
<div class='noprint'>100% local &middot; nothing leaves this Mac &middot; capture ledger stored in this browser profile localStorage &middot; Windows build &middot; <span class='kbd'>&#8984;K</span> commands &middot; <span class='kbd'>?</span> shortcuts.</div>
</footer>
...
sc=0;ev=[];
if(CAP.ssid==='Not available'&&ISWIFI){sc+=10;ev.push('SSID could not be read. This is usually a Location Services permission, not a fault.');}
...
if(CAP.loss===0&&CAP.drop===0&&(!CAP.haveSig||CAP.rssi>-70))return {t:'LINK HEALTHY',p:'NO FAULT OBSERVED',lamp:'lamp-ok',pill:'sp-ok',emo:String.fromCodePoint(128994)};
if(CAP.loss>=5||CAP.drop>=3)return {t:'LINK DEGRADED',p:top.sc>0?('LIKELY: '+top.k.toUpperCase()):'INVESTIGATE',lamp:'lamp-bad',pill:'sp-bad',emo:String.fromCodePoint(128308)};
if(CAP.loss>0||(CAP.haveSig&&CAP.rssi<=-72)||CAP.jit>30)return {t:'MARGINAL LINK',p:top.sc>0?('WATCH: '+top.k.toUpperCase()):'MONITOR',lamp:'lamp-warn',pill:'sp-warn',emo:String.fromCodePoint(128993)};
return {t:'LINK STABLE',p:'WITHIN TOLERANCE',lamp:'lamp-ok',pill:'sp-ok',emo:String.fromCodePoint(128994)};})();
var Chart={
...
rows.forEach(function(r,i){var y=20+i*gap;var t=(r[1]-r[2])/(r[3]-r[2]);if(t<0)t=0;if(t>1)t=1;
...
c.fillStyle=self.css('--txt');c.font='700 11px '+self.css('--mono');c.textAlign='right';var unit=' dBm';if(r[0]==='SNR')unit=' dB';if(r[0]==='QUALITY')unit='%';
c.fillText(r[1]+unit,w-4,y+13);});
c.fillStyle=this.css('--txt-mute');c.font='8px '+this.css('--mono');c.textAlign='left';c.fillText(HAVENOISE?'targets: RSSI better than -67 dBm, noise below -90 dBm, SNR above 25 dB':'targets: RSSI better than -67 dBm, link quality above 60 percent',4,h-8);},
...
defs:[{id:'verdict',ico:String.fromCodePoint(127919),title:'ROOT CAUSE MATRIX',wide:true,r:'rVerdict'},
...
build:function(){var self=this;q('#grid').innerHTML=this.visible().map(function(d){return '<section class="panel '+(d.wide?'wide':'')+(self.collapsed[d.id]?' collapsed':'')+'" id="panel_'+d.id+'"><div class="p-head" data-act="collapse" data-id="'+d.id+'"><h3><span class="caret">&#9662;</span><span>'+d.ico+'</span>'+d.title+'</h3><button class="p-act" data-act="hide" data-id="'+d.id+'">&#10005;</button></div><div class="p-body" id="body_'+d.id+'"></div></section>';}).join('');this.renderAll();},
...
var Render={
rVerdict:function(){var h='<div class="hint" style="margin-bottom:10px">Weighted ranking of where to investigate first, computed from this capture only. Bars show relative confidence, not probability.</div>';
...
else if(!HAVENOISE&&ISWIFI)h+='<div class="advice warn">Signal strength is acceptable at this position. '+esc(PLAT)+' does not expose a noise floor, so run the macOS capture on a Mac at the same desk if you need SNR to confirm or rule out interference.</div>';
...
return '<div class="hint" style="margin-bottom:9px">Pre-filled with the measurements from this capture. Paste straight into Copilot.</div><div class="row noprint" style="margin-bottom:9px"><button class="btn" data-act="copy1">COPY ROOT CAUSE PROMPT</button><button class="btn" data-act="copy2">COPY SERVICENOW PROMPT</button></div><pre>'+esc(p1)+'</pre><div style="height:9px"></div><pre>'+esc(p2)+'</pre>';},
rRaw:function(){var b=CAP.raw;var secs=[['Gateway test',b.gw],['DNS resolution',b.dns],['HTTPS timing',b.http],['Traceroute',b.trace],['Routing table',b.route],['Interface detail',b.ifc],['scutil DNS',b.scutil],['Preferred wireless networks',b.pref],['system_profiler SPAirPortDataType',b.air]];
...
exportJson:function(){var blob=new Blob([JSON.stringify({capture:CAP,scores:SCORES,ledger:CAPS},null,2)],{type:'application/json'});var a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='WiFiCapture_'+CAP.id+'.json';a.click();toast('Snapshot exported','ok');},
