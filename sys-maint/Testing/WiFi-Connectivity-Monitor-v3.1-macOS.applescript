-- ============================================================================
-- WIFI CONNECTIVITY MONITOR v3.1
-- 
-- 
--
...
--   A first-class panel written for non-technical readers. Every measurement
--   is explained with an everyday analogy and interpreted against the actual
--   captured value, plus a jargon decoder, a "what happens next" list and a
--   ready-to-send status message with a one-click copy button. A third
--   Copilot prompt generates an end-user summary on demand.
...
set locPick to choose from list locOptions with title "WiFi Connectivity Monitor v3.1" with prompt "Where is this capture being taken? Captures are stored and compared against each other in the dashboard." default items {"Desk / User Location"}
...
set modePick to choose from list modeOptions with title "WiFi Connectivity Monitor v3.1" with prompt "Select stability test length. Longer captures catch intermittent drops far more reliably." default items {"Standard - 60 samples (about 1 min)"}
...
my writeNew(logPath, "[" & (do shell script "date '+%Y-%m-%d %H:%M:%S'") & "] WiFi Connectivity Monitor v3.1 started" & linefeed)
...
set capJs to "{"
set capJs to capJs & "v:'3.1',"
set capJs to capJs & "plat:'macOS',"
...
set capJs to capJs & "haveNoise:" & (haveSignal as text) & ","
set capJs to capJs & "sigPct:null,"
...
set cssText to cssText & ".p-body{padding:12px;max-height:2400px;overflow:auto;transition:max-height .3s ease,padding .3s ease}" & linefeed
...
set cssText to cssText & ".xp{border-left:3px solid var(--cyan);background:rgba(0,229,255,.045);border-radius:8px;padding:12px 14px;margin-bottom:10px}" & linefeed
set cssText to cssText & ".xp.ok{border-left-color:var(--green);background:rgba(0,224,138,.05)}" & linefeed
set cssText to cssText & ".xp.warn{border-left-color:var(--amber);background:rgba(255,176,32,.05)}" & linefeed
set cssText to cssText & ".xp.bad{border-left-color:var(--red);background:rgba(255,59,87,.05)}" & linefeed
set cssText to cssText & ".xp h4{margin:0 0 6px;font-size:13.5px;color:var(--txt);font-weight:650}" & linefeed
set cssText to cssText & ".xp .an{font-style:italic;color:var(--txt-dim);font-size:12.5px;line-height:1.55;margin-bottom:8px}" & linefeed
set cssText to cssText & ".xp .rs{font-family:var(--mono);font-size:11px;letter-spacing:.06em;color:var(--cyan);margin-bottom:6px}" & linefeed
set cssText to cssText & ".xp.ok .rs{color:var(--green)}" & linefeed
set cssText to cssText & ".xp.warn .rs{color:var(--amber)}" & linefeed
set cssText to cssText & ".xp.bad .rs{color:var(--red)}" & linefeed
set cssText to cssText & ".xp .mn{font-size:12.5px;line-height:1.62;color:var(--txt)}" & linefeed
set cssText to cssText & ".vbox{border:1px solid var(--border);border-radius:11px;padding:16px 17px;margin-bottom:15px;background:rgba(0,229,255,.06)}" & linefeed
set cssText to cssText & ".vbox.ok{border-color:rgba(0,224,138,.5);background:rgba(0,224,138,.07)}" & linefeed
set cssText to cssText & ".vbox.warn{border-color:rgba(255,176,32,.5);background:rgba(255,176,32,.07)}" & linefeed
set cssText to cssText & ".vbox.bad{border-color:rgba(255,59,87,.5);background:rgba(255,59,87,.07)}" & linefeed
set cssText to cssText & ".vbox .vt{font-size:18px;font-weight:720;margin-bottom:7px;letter-spacing:-.2px}" & linefeed
set cssText to cssText & ".vbox .vs{font-size:13.5px;line-height:1.65}" & linefeed
set cssText to cssText & ".msgbox{border:1px dashed var(--border);border-radius:9px;padding:13px 15px;margin-top:10px;background:rgba(0,0,0,.24);font-size:12.5px;line-height:1.7;white-space:pre-wrap}" & linefeed
set cssText to cssText & "html[data-theme='light'] .msgbox{background:rgba(0,0,0,.04)}" & linefeed
set cssText to cssText & ".glos{display:grid;grid-template-columns:132px 1fr;gap:5px 12px;font-size:12.5px;line-height:1.55;margin-top:4px}" & linefeed
set cssText to cssText & ".glos b{color:var(--cyan);font-family:var(--mono);font-size:11px;letter-spacing:.04em;padding-top:2px}" & linefeed
set cssText to cssText & "@media(max-width:600px){.glos{grid-template-columns:1fr;gap:2px 0}.glos b{padding-top:8px}}" & linefeed
...
set shellText to shellText & "<div class='brand'><span class='bolt'>&#128225;</span><span class='b1'>WIFI</span><span class='b2'>CONNECTIVITY MONITOR</span><span class='ver'>v3.1</span></div>" & linefeed
set shellText to shellText & "<span class='live'><span class='dot'></span>CAPTURED</span>" & linefeed
set shellText to shellText & "<span class='statuspill sp-ok' id='platPill'>PLATFORM</span>" & linefeed
...
set shellText to shellText & "<button class='tb-btn' data-act='explain' title='Plain English briefing (X)'>&#128172; EXPLAIN</button>" & linefeed
...
set shellText to shellText & "<div><b class='c-cyan'>WIFI CONNECTIVITY MONITOR v3.1</b> &middot; IT Support &middot; read-only capture, no network settings were changed.</div>" & linefeed
set shellText to shellText & "<div>New in v3.1: a <b>Plain English Briefing</b> panel that explains every measurement in everyday language with analogies, plus a ready-to-send status message. Share this report with non-technical colleagues.</div>" & linefeed
...
set shellText to shellText & "<div id='platNote'></div>" & linefeed
set shellText to shellText & "<div class='noprint'>100% local &middot; nothing leaves this machine &middot; capture ledger stored in this browser profile localStorage &middot; <span class='kbd'>&#8984;K</span> commands &middot; <span class='kbd'>?</span> shortcuts.</div>" & linefeed
set shellText to shellText & "</footer>" & linefeed
...
set shellText to shellText & "<tr><td><span class='kbd'>X</span></td><td class='dim'>Jump to the Plain English Briefing</td></tr>" & linefeed
...
set shellText to shellText & "<tr><td><span class='kbd'>M</span></td><td class='dim'>Copy the ready-to-send status message</td></tr>" & linefeed
...
set jsText to jsText & "var HAVENOISE=(CAP.haveNoise===undefined)?CAP.haveSig:CAP.haveNoise;" & linefeed
set jsText to jsText & "var PLAT=CAP.plat||'macOS';" & linefeed
set jsText to jsText & "var HASPCT=(CAP.sigPct!==undefined&&CAP.sigPct!==null);" & linefeed
set jsText to jsText & "var Ledger={all:function(){return LS.get('caps',[]);}," & linefeed
set jsText to jsText & "save:function(){var a=this.all();a=a.filter(function(x){return x.id!==CAP.id;});a.push({id:CAP.id,ts:CAP.ts,loc:CAP.loc,link:CAP.link,ssid:CAP.ssid,chan:CAP.chan,rssi:CAP.rssi,snr:CAP.snr,haveSig:CAP.haveSig,haveNoise:CAP.haveNoise,plat:CAP.plat,loss:CAP.loss,avg:CAP.avg,max:CAP.max,jit:CAP.jit,drop:CAP.drop,nbTotal:CAP.nbTotal,gw:CAP.gw,n:CAP.n});a.sort(function(x,y){return x.ts-y.ts;});if(a.length>14)a=a.slice(a.length-14);LS.set('caps',a);return a;}," & linefeed
...
set jsText to jsText & "if(HAVENOISE&&CAP.snr>0&&CAP.snr<25){sc+=45;ev.push('SNR is '+CAP.snr+' dB. Healthy is 25 dB or better, so the noise floor here is elevated.');}" & linefeed
set jsText to jsText & "else if(HAVENOISE&&CAP.snr>=25&&CAP.snr<32){sc+=18;ev.push('SNR '+CAP.snr+' dB is workable but not comfortable.');}" & linefeed
set jsText to jsText & "else if(!HAVENOISE&&ISWIFI){ev.push('This platform does not report a noise floor, so SNR could not be measured. Interference is scored from band, channel density and jitter only.');}" & linefeed
...
set jsText to jsText & "sc=0;ev=[];" & linefeed
set jsText to jsText & "if(CAP.ssid==='Not available'&&ISWIFI){sc+=10;ev.push('SSID could not be read. This is usually a permissions or driver reporting quirk, not a fault.');}" & linefeed
...
set jsText to jsText & "if(CAP.loss===0&&CAP.drop===0&&(!CAP.haveSig||CAP.rssi>-70))return {t:'LINK HEALTHY',p:'NO FAULT OBSERVED',lamp:'lamp-ok',pill:'sp-ok',emo:String.fromCodePoint(128994),cls:'ok'};" & linefeed
set jsText to jsText & "if(CAP.loss>=5||CAP.drop>=3)return {t:'LINK DEGRADED',p:top.sc>0?('LIKELY: '+top.k.toUpperCase()):'INVESTIGATE',lamp:'lamp-bad',pill:'sp-bad',emo:String.fromCodePoint(128308),cls:'bad'};" & linefeed
set jsText to jsText & "if(CAP.loss>0||(CAP.haveSig&&CAP.rssi<=-72)||CAP.jit>30)return {t:'MARGINAL LINK',p:top.sc>0?('WATCH: '+top.k.toUpperCase()):'MONITOR',lamp:'lamp-warn',pill:'sp-warn',emo:String.fromCodePoint(128993),cls:'warn'};" & linefeed
set jsText to jsText & "return {t:'LINK STABLE',p:'WITHIN TOLERANCE',lamp:'lamp-ok',pill:'sp-ok',emo:String.fromCodePoint(128994),cls:'ok'};})();" & linefeed
...
set jsText to jsText & "var rows=[['RSSI',CAP.rssi,-95,-35,[-72,-67]]];" & linefeed
set jsText to jsText & "if(HAVENOISE){rows.push(['NOISE',CAP.noise,-100,-40,[-90,-80]]);rows.push(['SNR',CAP.snr,0,50,[20,25]]);}" & linefeed
set jsText to jsText & "if(HASPCT){rows.push(['QUALITY',CAP.sigPct,0,100,[40,60]]);}" & linefeed
set jsText to jsText & "var barH=18,gap=28,x0=54,bw=w-x0-64;" & linefeed
set jsText to jsText & "rows.forEach(function(r,i){var y=16+i*gap;var t=(r[1]-r[2])/(r[3]-r[2]);if(t<0)t=0;if(t>1)t=1;" & linefeed
...
set jsText to jsText & "else if(r[0]==='QUALITY'){if(r[1]<r[4][0])col=self.css('--red');else if(r[1]<r[4][1])col=self.css('--amber');}" & linefeed
...
set jsText to jsText & "c.fillStyle=self.css('--txt');c.font='700 11px '+self.css('--mono');c.textAlign='right';" & linefeed
set jsText to jsText & "var unit=' dBm';if(r[0]==='SNR')unit=' dB';if(r[0]==='QUALITY')unit='%';" & linefeed
set jsText to jsText & "c.fillText(r[1]+unit,w-4,y+13);});" & linefeed
set jsText to jsText & "c.fillStyle=this.css('--txt-mute');c.font='8px '+this.css('--mono');c.textAlign='left';" & linefeed
set jsText to jsText & "c.fillText(HAVENOISE?'targets: RSSI better than -67 dBm, noise below -90 dBm, SNR above 25 dB':'targets: RSSI better than -67 dBm, link quality above 60 percent',4,h-6);}," & linefeed
...
set jsText to jsText & "var Plain={" & linefeed
set jsText to jsText & "grade:function(){" & linefeed
set jsText to jsText & " if(CAP.loss===0&&CAP.drop===0)return 'ok';" & linefeed
set jsText to jsText & " if(CAP.loss>=5||CAP.drop>=3)return 'bad';" & linefeed
set jsText to jsText & " return 'warn';}," & linefeed
set jsText to jsText & "headline:function(){" & linefeed
set jsText to jsText & " var g=this.grade();" & linefeed
set jsText to jsText & " if(g==='ok')return {t:'Your connection looked healthy during this test.',s:'We sent '+CAP.n+' small test messages out to the internet and every single one came back. Nothing was lost and nothing was interrupted while we were watching.'};" & linefeed
set jsText to jsText & " if(g==='bad')return {t:'Your connection is genuinely dropping out. This is not your imagination.',s:'We sent '+CAP.n+' small test messages out to the internet. '+CAP.bad+' of them never came back, and at one point '+CAP.drop+' in a row went missing. That gap is exactly the moment a call freezes or a page stops loading.'};" & linefeed
set jsText to jsText & " return {t:'Your connection is mostly working, but it is not solid.',s:'We sent '+CAP.n+' small test messages out to the internet and '+CAP.bad+' did not come back. That is not enough to break everything, but it is enough to cause the occasional stutter on a call or a page that hangs for a second.'};}," & linefeed
set jsText to jsText & "plainCause:function(){" & linefeed
set jsText to jsText & " var top=SCORES[0];" & linefeed
set jsText to jsText & " if(top.sc<=0)return 'Nothing in this test points to a specific fault. If it still misbehaves, we need to capture again while the problem is actually happening.';" & linefeed
set jsText to jsText & " var m={" & linefeed
set jsText to jsText & "  'RF Interference / Noise':'The airwaves around you are crowded. Your Wi-Fi is competing with a lot of other signals nearby.'," & linefeed
set jsText to jsText & "  'Access Point Coverage / Placement':'You are too far from the Wi-Fi equipment, or something solid is in the way.'," & linefeed
set jsText to jsText & "  'Roaming / Association Loss':'Your device is briefly letting go of the Wi-Fi and having to reconnect.'," & linefeed
set jsText to jsText & "  'Local Router / Gateway':'The problem starts at the office box that connects everyone to the internet.'," & linefeed
set jsText to jsText & "  'DNS / Name Resolution':'Your device is struggling to look up website addresses.'," & linefeed
set jsText to jsText & "  'ISP / Upstream Circuit':'The office equipment looks fine. The trouble is on the internet line coming into the building.'," & linefeed
set jsText to jsText & "  'Endpoint / Client Device':'This looks specific to this one computer rather than the office network.'};" & linefeed
set jsText to jsText & " return m[top.k]||top.k;}," & linefeed
set jsText to jsText & "message:function(){" & linefeed
set jsText to jsText & " var g=this.grade(),L=[];" & linefeed
set jsText to jsText & " L.push('Hi,');" & linefeed
set jsText to jsText & " L.push('');" & linefeed
set jsText to jsText & " if(g==='ok'){" & linefeed
set jsText to jsText & "  L.push('I ran a network test on the affected machine at the ' + CAP.loc.toLowerCase() + ' and the connection held up cleanly the whole time. Out of ' + CAP.n + ' test messages, none were lost.');" & linefeed
set jsText to jsText & "  L.push('');" & linefeed
set jsText to jsText & "  L.push('That does not mean the problem is imaginary. It means it was not happening during the window I measured. The next step is to run the same test while the issue is actually occurring so I can catch it in the act.');" & linefeed
set jsText to jsText & " } else if(g==='bad'){" & linefeed
set jsText to jsText & "  L.push('I ran a network test on the affected machine at the ' + CAP.loc.toLowerCase() + '. It confirms what you have been experiencing.');" & linefeed
set jsText to jsText & "  L.push('');" & linefeed
set jsText to jsText & "  L.push('Out of ' + CAP.n + ' test messages sent to the internet, ' + CAP.bad + ' never came back, and at one point ' + CAP.drop + ' in a row were lost. That is the moment a call drops or a page stops loading.');" & linefeed
set jsText to jsText & "  L.push('');" & linefeed
set jsText to jsText & "  L.push('Most likely cause: ' + this.plainCause());" & linefeed
set jsText to jsText & " } else {" & linefeed
set jsText to jsText & "  L.push('I ran a network test on the affected machine at the ' + CAP.loc.toLowerCase() + '. The connection is working, but it is not as steady as it should be.');" & linefeed
set jsText to jsText & "  L.push('');" & linefeed
set jsText to jsText & "  L.push('Out of ' + CAP.n + ' test messages, ' + CAP.bad + ' did not come back. That is enough to cause an occasional stutter, though not a total outage.');" & linefeed
set jsText to jsText & "  L.push('');" & linefeed
set jsText to jsText & "  L.push('Most likely cause: ' + this.plainCause());" & linefeed
set jsText to jsText & " }" & linefeed
set jsText to jsText & " L.push('');" & linefeed
set jsText to jsText & " L.push('Next step: ' + (g==='ok'?'re-test during the next reported failure.':'I will repeat this test in a couple of other spots so we can pin down exactly where it breaks down.'));" & linefeed
set jsText to jsText & " L.push('');" & linefeed
set jsText to jsText & " L.push('I will keep you posted.');" & linefeed
set jsText to jsText & " L.push('');" & linefeed
set jsText to jsText & " L.push('Thank you,');" & linefeed
set jsText to jsText & " L.push('Full Name');" & linefeed
set jsText to jsText & " L.push('IT Support');" & linefeed
set jsText to jsText & " return L.join(String.fromCharCode(10));}};" & linefeed
set jsText to jsText & "var Panels={hidden:LS.get('hidden',{}),collapsed:LS.get('collapsed',{})," & linefeed
set jsText to jsText & "defs:[{id:'explain',ico:String.fromCodePoint(128172),title:'PLAIN ENGLISH BRIEFING',wide:true,r:'rExplain'}," & linefeed
set jsText to jsText & "{id:'verdict',ico:String.fromCodePoint(127919),title:'ROOT CAUSE MATRIX',wide:true,r:'rVerdict'}," & linefeed
...
set jsText to jsText & "build:function(){var self=this;q('#grid').innerHTML=this.visible().map(function(d){return '<section class=\"panel '+(d.wide?'wide':'')+(self.collapsed[d.id]?' collapsed':'')+'\" id=\"panel_'+d.id+'\"><div class=\"p-head\" data-act=\"collapse\" data-id=\"'+d.id+'\"><h3><span class=\"caret\">&#9662;</span><span>'+d.ico+'</span>'+d.title+(d.id==='explain'?' <span class=\"badge new\">FOR EVERYONE</span>':'')+'</h3><button class=\"p-act\" data-act=\"hide\" data-id=\"'+d.id+'\">&#10005;</button></div><div class=\"p-body\" id=\"body_'+d.id+'\"></div></section>';}).join('');this.renderAll();}," & linefeed
...
set jsText to jsText & "rExplain:function(){" & linefeed
set jsText to jsText & " function blk(cls,title,analogy,result,meaning){return '<div class=\"xp '+cls+'\"><h4>'+esc(title)+'</h4><div class=\"an\">'+esc(analogy)+'</div><div class=\"rs\">YOUR RESULT: '+esc(result)+'</div><div class=\"mn\">'+meaning+'</div></div>';}" & linefeed
set jsText to jsText & " var hd=Plain.headline(),g=Plain.grade();" & linefeed
set jsText to jsText & " var h='<div class=\"hint\" style=\"margin-bottom:12px\">This section explains the whole report in everyday language. No technical background needed. Every number below is from the test that just ran on this machine.</div>';" & linefeed
set jsText to jsText & " h+='<div class=\"vbox '+g+'\"><div class=\"vt\">'+esc(hd.t)+'</div><div class=\"vs\">'+esc(hd.s)+'</div></div>';" & linefeed
set jsText to jsText & " h+='<div class=\"hint\" style=\"margin:14px 0 9px\">WHAT EACH MEASUREMENT MEANS</div>';" & linefeed
set jsText to jsText & " var c1=CAP.loss===0?'ok':(CAP.loss>=5?'bad':'warn');" & linefeed
set jsText to jsText & " h+=blk(c1,'Packet loss - did anything go missing?'," & linefeed
set jsText to jsText & "  'Think of posting ' + CAP.n + ' letters and counting how many replies come back. Every letter should get a reply. Any that go missing are gone for good, and the internet simply gives up on them.'," & linefeed
set jsText to jsText & "  CAP.bad + ' of ' + CAP.n + ' went missing (' + CAP.loss + '%)'," & linefeed
set jsText to jsText & "  CAP.loss===0?'Nothing was lost. This is what a healthy connection looks like.':(CAP.loss>=5?'This is a lot. At this level video calls freeze, audio breaks up and files fail to upload. This is the single clearest sign that something is genuinely wrong.':'A small amount went missing. You would notice this as an occasional hiccup rather than a full outage, but it should be zero.'));" & linefeed
set jsText to jsText & " var c2=CAP.avg<=40?'ok':(CAP.avg>90?'bad':'warn');" & linefeed
set jsText to jsText & " h+=blk(c2,'Latency - how long does a reply take?'," & linefeed
set jsText to jsText & "  'This is how quickly someone answers when you call their name across the office. Lower is better. Anything under about 40 milliseconds feels instant to a human being.'," & linefeed
set jsText to jsText & "  'average ' + CAP.avg + ' ms (fastest ' + CAP.min + ', slowest ' + CAP.max + ')'," & linefeed
set jsText to jsText & "  CAP.avg<=40?'This is quick. Nothing here would make anything feel sluggish.':(CAP.avg>90?'This is slow enough to feel. Pages hesitate before loading and people talk over each other on calls because the delay makes it hard to judge when someone has finished.':'A little slower than ideal, but most people would not consciously notice this on its own.'));" & linefeed
set jsText to jsText & " var c3=CAP.jit<=15?'ok':(CAP.jit>30?'bad':'warn');" & linefeed
set jsText to jsText & " h+=blk(c3,'Jitter - is the rhythm steady?'," & linefeed
set jsText to jsText & "  'Imagine a bus that is supposed to come every ten minutes. If it arrives every ten minutes exactly, you can plan around it. If it arrives after two minutes, then eighteen, then five, the average is still ten but it is useless. Jitter measures that unevenness.'," & linefeed
set jsText to jsText & "  CAP.jit + ' ms of variation between messages'," & linefeed
set jsText to jsText & "  CAP.jit<=15?'The rhythm is steady. Voice and video have a smooth stream of data to work with.':(CAP.jit>30?'This is uneven, and it is the usual reason voices sound robotic or chopped up on Zoom and Teams even when the internet speed test looks fine. Speed is not the problem here; consistency is.':'Slightly uneven. Occasionally noticeable on a call, but not severe.'));" & linefeed
set jsText to jsText & " var c4=CAP.drop===0?'ok':(CAP.drop>=3?'bad':'warn');" & linefeed
set jsText to jsText & " h+=blk(c4,'Longest unbroken gap - did it actually cut out?'," & linefeed
set jsText to jsText & "  'There is a difference between a phone call that sounds a bit muffled and one where the line goes completely dead for four seconds. This measures the dead silence, not the muffling.'," & linefeed
set jsText to jsText & "  CAP.drop===0?'no gaps at all':CAP.drop + ' messages lost back to back'," & linefeed
set jsText to jsText & "  CAP.drop===0?'The connection never actually let go. Whatever else is going on, it did not fully cut out during this test.':(CAP.drop>=3?'This is a real disconnection, not just slowness. This is the moment someone says \"I lost you\" on a call. It is the most important finding in this report.':'A very brief interruption. Probably not noticeable by itself.'));" & linefeed
set jsText to jsText & " if(ISWIFI&&CAP.haveSig){" & linefeed
set jsText to jsText & "  var c5=CAP.rssi>-67?'ok':(CAP.rssi<=-72?'bad':'warn');" & linefeed
set jsText to jsText & "  h+=blk(c5,'Signal strength - how clearly can your device hear the Wi-Fi?'," & linefeed
set jsText to jsText & "   'Picture someone speaking to you from across a room. Up close they are easy to hear. From the far end, through a closed door, you catch maybe half the words and have to keep asking them to repeat themselves. Your laptop has the same problem with the Wi-Fi box.'," & linefeed
set jsText to jsText & "   CAP.rssi + ' dBm' + (HASPCT?(' (about ' + CAP.sigPct + '% quality)'):'') + ' on channel ' + esc(CAP.chan) + ', ' + BAND," & linefeed
set jsText to jsText & "   CAP.rssi>-67?'Strong. Your device is hearing the Wi-Fi clearly from where it is sitting.':(CAP.rssi<=-72?'Weak. Your device is straining to hear the Wi-Fi from this spot. The simplest test is to sit closer to the Wi-Fi box and see if things improve immediately.':'Usable but not strong. Moving closer would help.'));" & linefeed
set jsText to jsText & " }" & linefeed
set jsText to jsText & " if(ISWIFI&&HAVENOISE){" & linefeed
set jsText to jsText & "  var c6=CAP.snr>=25?'ok':'warn';" & linefeed
set jsText to jsText & "  h+=blk(c6,'Background noise - how noisy is the room?'," & linefeed
set jsText to jsText & "   'You can hear someone perfectly in a quiet library. Say the exact same words at the same volume in a busy restaurant and you catch nothing. The voice did not get quieter; the background got louder. This measures the background.'," & linefeed
set jsText to jsText & "   'signal is ' + CAP.snr + ' dB above the background noise'," & linefeed
set jsText to jsText & "   CAP.snr>=25?'Comfortably above the noise. Your device can pick out the Wi-Fi easily.':'The background is loud relative to your signal. This is the classic fingerprint of a shared office building, where all the neighbouring businesses are competing for the same airwaves.');" & linefeed
set jsText to jsText & " }" & linefeed
set jsText to jsText & " if(ISWIFI){" & linefeed
set jsText to jsText & "  var c7=CAP.nbTotal>12?'bad':(CAP.nbTotal>6?'warn':'ok');" & linefeed
set jsText to jsText & "  h+=blk(c7,'Neighbouring networks - how many others are competing?'," & linefeed
set jsText to jsText & "   'Wi-Fi channels are like lanes on a road. If you have a lane to yourself you move freely. If forty cars are merging into your lane from the businesses on either side, everyone crawls, no matter how good your car is.'," & linefeed
set jsText to jsText & "   CAP.nbTotal + ' other networks detected nearby, you are on channel ' + CHAN + ' (' + BAND + ')'," & linefeed
set jsText to jsText & "   CAP.nbTotal>12?'That is a crowded neighbourhood. In a shared building this is one of the most common reasons Wi-Fi is fine one minute and unusable the next. The fix is usually changing which lane the office Wi-Fi uses, which the landlord or network vendor has to do.':(CAP.nbTotal>6?'A moderate amount of competition. Worth noting, not alarming on its own.':'Not many competitors nearby. Congestion is unlikely to be your problem.'));" & linefeed
set jsText to jsText & " }" & linefeed
set jsText to jsText & " var c8=CAP.gw==='clean'?'ok':(CAP.gw==='unknown'?'warn':'bad');" & linefeed
set jsText to jsText & " h+=blk(c8,'The office box - is the problem inside or outside the building?'," & linefeed
set jsText to jsText & "  'Every office has one box that everything goes through on its way to the internet, like the single front door of a building. We knocked on that door separately. If the door answers fine but your post still goes missing, the problem is out on the road, not in the building.'," & linefeed
set jsText to jsText & "  CAP.gw==='clean'?'the office box answered every time':(CAP.gw==='lossy'?'the office box answered, but not reliably':'the office box did not answer properly')," & linefeed
set jsText to jsText & "  CAP.gw==='clean'?(CAP.loss>0?'The office box itself is healthy, so the trouble is further out - either the Wi-Fi in the air between you and it, or the internet line leaving the building.':'The office box is healthy and nothing was lost. Everything checks out here.'):'The office box is not responding cleanly, which means the problem starts inside the building. That points at the router or the local network equipment rather than the internet provider.');" & linefeed
set jsText to jsText & " var c9=CAP.dnsOk===1?'ok':'bad';" & linefeed
set jsText to jsText & " h+=blk(c9,'Address lookup - can your device find websites by name?'," & linefeed
set jsText to jsText & "  'You know the name of the person you want to call, but you still need the phone book to get their number. Your device does the same thing every time you type a website name. If the phone book is missing, everything looks broken even when the connection itself is perfect.'," & linefeed
set jsText to jsText & "  CAP.dnsOk===1?'lookups are working normally':'lookups did not come back'," & linefeed
set jsText to jsText & "  CAP.dnsOk===1?'Your device can find websites by name without any trouble.':'This alone can make the internet appear completely dead even when the connection is fine. It is often a quick fix and is worth checking before anything else.');" & linefeed
set jsText to jsText & " h+='<div class=\"hint\" style=\"margin:16px 0 9px\">WHAT HAPPENS NEXT</div>';" & linefeed
set jsText to jsText & " var nx=[];" & linefeed
set jsText to jsText & " if(ISWIFI&&CAP.loss>0)nx.push('Run this same test on a cable instead of Wi-Fi from the same desk. That one comparison tells us whether to chase the Wi-Fi or the internet line, and it saves days of guessing.');" & linefeed
set jsText to jsText & " if(ISWIFI&&CAP.haveSig&&CAP.rssi<=-72)nx.push('Run the test again sitting right next to the Wi-Fi box. If the numbers jump, it is a coverage problem and the answer is another access point, not a new laptop.');" & linefeed
set jsText to jsText & " if(!ISWIFI&&CAP.loss>0)nx.push('This test ran on a cable and still lost data, which means the Wi-Fi is not to blame. This becomes a conversation with the internet provider, and this report is the evidence.');" & linefeed
set jsText to jsText & " if(CAP.nbTotal>12)nx.push('Share the neighbouring network count with the building or the network vendor. It is the justification for changing the Wi-Fi channel plan.');" & linefeed
set jsText to jsText & " if(CAP.loss===0&&CAP.drop===0)nx.push('Run this again the moment the problem happens next. A clean test only proves it was behaving while we watched, and catching it misbehaving is what we need.');" & linefeed
set jsText to jsText & " nx.push('Keep this report. Every test is saved and compared automatically, so the picture gets clearer with each one rather than starting from scratch.');" & linefeed
set jsText to jsText & " h+='<ul style=\"margin:0;padding-left:18px\">'+nx.map(function(x){return '<li style=\"margin:7px 0;line-height:1.6\">'+esc(x)+'</li>';}).join('')+'</ul>';" & linefeed
set jsText to jsText & " h+='<div class=\"hint\" style=\"margin:16px 0 9px\">READY-TO-SEND UPDATE</div>';" & linefeed
set jsText to jsText & " h+='<div class=\"hint\" style=\"margin-bottom:8px\">Written for a non-technical reader. Copy it straight into Zoom, Teams or email.</div>';" & linefeed
set jsText to jsText & " h+='<div class=\"row noprint\" style=\"margin-bottom:8px\"><button class=\"btn\" data-act=\"copyMsg\">COPY THIS MESSAGE</button></div>';" & linefeed
set jsText to jsText & " h+='<div class=\"msgbox\">'+esc(Plain.message())+'</div>';" & linefeed
set jsText to jsText & " h+='<div class=\"hint\" style=\"margin:16px 0 9px\">JARGON DECODER</div>';" & linefeed
set jsText to jsText & " h+='<div class=\"glos\">'+" & linefeed
set jsText to jsText & "  '<b>Packet</b><span>One small piece of internet data. Everything you do online is chopped into thousands of these.</span>'+" & linefeed
set jsText to jsText & "  '<b>Packet loss</b><span>Pieces that never arrived. The percentage of things that went missing.</span>'+" & linefeed
set jsText to jsText & "  '<b>Latency / ms</b><span>Delay, measured in milliseconds. One thousand milliseconds is one second.</span>'+" & linefeed
set jsText to jsText & "  '<b>Jitter</b><span>How uneven the delay is. Steady is good, erratic is bad, even at the same average speed.</span>'+" & linefeed
set jsText to jsText & "  '<b>RSSI / dBm</b><span>How loudly your device hears the Wi-Fi. Closer to zero is stronger, so -55 is much better than -85.</span>'+" & linefeed
set jsText to jsText & "  (HAVENOISE?'<b>SNR</b><span>How far your signal stands above the background noise. Higher is clearer.</span>':'')+" & linefeed
set jsText to jsText & "  '<b>Channel</b><span>Which lane the Wi-Fi is using. Neighbours on the same lane slow each other down.</span>'+" & linefeed
set jsText to jsText & "  '<b>Gateway</b><span>The office box everything passes through on the way to the internet.</span>'+" & linefeed
set jsText to jsText & "  '<b>DNS</b><span>The internet phone book that turns a website name into an address.</span>'+" & linefeed
set jsText to jsText & "  '<b>Access point</b><span>The Wi-Fi transmitter on the wall or ceiling that your device connects to.</span>'+" & linefeed
set jsText to jsText & "  '</div>';" & linefeed
set jsText to jsText & " return h;}," & linefeed
set jsText to jsText & "rVerdict:function(){var h='<div class=\"hint\" style=\"margin-bottom:10px\">Weighted ranking of where to investigate first, computed from this capture only. Bars show relative confidence, not probability. For a non-technical explanation, see the Plain English Briefing panel.</div>';" & linefeed
...
set jsText to jsText & "else if(HAVENOISE&&CAP.snr<25)h+='<div class=\"advice warn\">Signal strength is acceptable but the noise floor is high, which is the classic signature of a shared multi-tenant building.</div>';" & linefeed
set jsText to jsText & "else if(!HAVENOISE&&ISWIFI)h+='<div class=\"advice warn\">Signal strength is acceptable at this position. '+esc(PLAT)+' does not expose a noise floor, so run the macOS capture on a Mac at the same desk if you need SNR to confirm or rule out interference.</div>';" & linefeed
...
set jsText to jsText & "else if(BAND==='2.4 GHz')h+='<div class=\"advice warn\">The client is associated on 2.4 GHz. Steer these clients to 5 GHz. 2.4 GHz has only three non-overlapping channels and is shared with every neighbour, microwave and Bluetooth device nearby.</div>';" & linefeed
...
set jsText to jsText & "h+='<tr class=\"'+cls+'\"><td><b>'+esc(x.loc)+'</b>'+(x.id===CAP.id?' <span class=\"badge new\">THIS</span>':'')+'<br><span class=\"mute\" style=\"font-size:9px\">'+d.toLocaleDateString()+' '+d.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})+(x.plat?' &middot; '+esc(x.plat):'')+'</span></td><td class=\"dim\">'+esc(x.link)+'</td><td style=\"text-align:right\" class=\"'+(x.loss>0?'c-red':'c-green')+'\"><b>'+x.loss+'%</b></td><td style=\"text-align:right\">'+x.avg+' ms</td><td style=\"text-align:right\">'+x.jit+' ms</td><td style=\"text-align:right\">'+(x.haveSig?x.rssi:'--')+'</td><td style=\"text-align:right\">'+((x.haveNoise===undefined?x.haveSig:x.haveNoise)?x.snr:'--')+'</td><td style=\"text-align:right\" class=\"'+(x.drop>=3?'c-red':'')+'\">'+x.drop+'</td></tr>';});" & linefeed
...
set jsText to jsText & "if(ww===0&&fw>0)h+='<div class=\"advice bad\"><b>Wired is clean, Wi-Fi is not.</b> The fault is wireless or environmental - access point coverage, RF interference or channel congestion. The circuit and the endpoint devices are exonerated.</div>';" & linefeed
...
set jsText to jsText & "if(BAND==='2.4 GHz')a.push('Move the affected clients onto the 5 GHz SSID, or enable band steering if the access point supports it.');" & linefeed
...
set jsText to jsText & "a.push('Send the Plain English Briefing to the requester so they understand the finding without needing a translation.');" & linefeed
...
set jsText to jsText & "var sigTxt=HAVENOISE?(', noise '+CAP.noise+' dBm, SNR '+CAP.snr+' dB'):(HASPCT?(', link quality '+CAP.sigPct+' percent, noise floor and SNR are not reported on '+PLAT):(', noise floor and SNR are not reported on '+PLAT));" & linefeed
...
set jsText to jsText & "var p2='Using the capture data below, write a ServiceNow incident update for Executive Support. Include an Executive Impact and Business Value statement, a technical Work Notes section listing the investigation performed and the measurements taken, a Resolution or Action Plan with clear next steps, and a Status line. Then write a short, warm, non-technical message for the executive assistant explaining what was found and what happens next, with no IT jargon. Capture: point '+CAP.loc+', link '+CAP.link+', platform '+PLAT+', packet loss '+CAP.loss+' percent, average latency '+CAP.avg+' ms, jitter '+CAP.jit+' ms, longest outage '+CAP.drop+' samples, RSSI '+CAP.rssi+' dBm'+(HAVENOISE?(', SNR '+CAP.snr+' dB'):'')+', '+CAP.nbTotal+' neighbouring networks, channel '+CAP.chan+', gateway status '+CAP.gw+', top ranked cause '+SCORES[0].k+'.';" & linefeed
set jsText to jsText & "var p3='Explain this network test result to someone with no technical background at all. Use plain everyday language and a simple analogy for each measurement, the way you would explain it to a busy executive who just wants to know whether it is fixed. Do not use jargon, and if you must use a term, explain it in the same sentence. Keep it under 250 words, warm and reassuring in tone, and end with what happens next. The results: out of '+CAP.n+' test messages sent to the internet, '+CAP.bad+' were lost ('+CAP.loss+' percent). Average response time '+CAP.avg+' ms, with the slowest at '+CAP.max+' ms. Unevenness between messages (jitter) '+CAP.jit+' ms. The longest run of consecutive lost messages was '+CAP.drop+'. '+(ISWIFI?('Wi-Fi signal strength '+CAP.rssi+' dBm on channel '+CAP.chan+' ('+BAND+'), with '+CAP.nbTotal+' other networks detected nearby. '):'This test ran over a wired cable rather than Wi-Fi. ')+'The office gateway status was '+CAP.gw+' and address lookups '+(CAP.dnsOk===1?'worked':'failed')+'. The most likely cause identified was '+SCORES[0].k+'. Location tested: '+CAP.loc+'.';" & linefeed
set jsText to jsText & "window.__P1=p1;window.__P2=p2;window.__P3=p3;" & linefeed
set jsText to jsText & "return '<div class=\"hint\" style=\"margin-bottom:9px\">Pre-filled with the measurements from this capture. Paste straight into Copilot.</div><div class=\"row noprint\" style=\"margin-bottom:9px\"><button class=\"btn\" data-act=\"copy1\">COPY ROOT CAUSE PROMPT</button><button class=\"btn\" data-act=\"copy2\">COPY SERVICENOW PROMPT</button><button class=\"btn\" data-act=\"copy3\">COPY PLAIN ENGLISH PROMPT</button></div><div class=\"hint\" style=\"margin-bottom:5px\">1. ROOT CAUSE ANALYSIS</div><pre>'+esc(p1)+'</pre><div class=\"hint\" style=\"margin:11px 0 5px\">2. SERVICENOW WORK NOTES</div><pre>'+esc(p2)+'</pre><div class=\"hint\" style=\"margin:11px 0 5px\">3. PLAIN ENGLISH SUMMARY FOR THE END USER</div><pre>'+esc(p3)+'</pre>';}," & linefeed
set jsText to jsText & "rRaw:function(){var b=CAP.raw;var secs=[['Gateway test',b.gw],['DNS resolution',b.dns],['HTTPS timing',b.http],['Traceroute',b.trace],['Routing table',b.route],['Interface detail',b.ifc],['Host network configuration',b.scutil],['Preferred wireless networks',b.pref],['Adapter and driver detail',b.air]];" & linefeed
...
set jsText to jsText & "this.items.push({t:'Copy the ready-to-send status message',s:'M',f:function(){App.copy(Plain.message(),'Status message copied');}});" & linefeed
...
set jsText to jsText & "this.items.push({t:'Copy plain English prompt',s:'',f:function(){App.copy(window.__P3,'Plain English prompt copied');}});" & linefeed
...
set jsText to jsText & "exportJson:function(){var blob=new Blob([JSON.stringify({capture:CAP,scores:SCORES,ledger:CAPS,plainSummary:Plain.message()},null,2)],{type:'application/json'});var a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='WiFiCapture_'+CAP.id+'.json';a.click();toast('Snapshot exported','ok');}," & linefeed
...
set jsText to jsText & "var pp=q('#platPill');if(pp)pp.textContent=PLAT.toUpperCase();" & linefeed
set jsText to jsText & "var pn=q('#platNote');if(pn)pn.innerHTML=HAVENOISE?'Signal, channel and neighbour data are read from system_profiler SPAirPortDataType. If SSID reads as unavailable, grant this app Location Services access - all other metrics are unaffected.':'Signal, channel and neighbour data are read from netsh wlan. Windows reports signal as a link-quality percentage; RSSI is derived from it using the standard quality-to-dBm conversion. Windows does not expose a noise floor, so SNR is not shown - run the macOS capture on a Mac at the same desk when SNR is needed.';" & linefeed
...
set jsText to jsText & "HAVENOISE?['SNR',CAP.snr+' dB',CAP.snr>=25?'clean':'noisy',CAP.snr>=25?'--green':'--amber']:['LINK QUALITY',HASPCT?CAP.sigPct+'%':'--',HASPCT?(CAP.sigPct>=60?'good':'weak'):'SNR not reported',HASPCT?(CAP.sigPct>=60?'--green':'--amber'):'--txt-mute']," & linefeed
...
set jsText to jsText & "setTimeout(function(){toast('New: open the Plain English Briefing panel to share these results with a non-technical colleague.','ok',7000);},1400);}};" & linefeed
...
set jsText to jsText & "else if(a==='explain')Panels.focus('explain');" & linefeed
...
set jsText to jsText & "else if(a==='copy3')App.copy(window.__P3,'Plain English prompt copied');" & linefeed
set jsText to jsText & "else if(a==='copyMsg')App.copy(Plain.message(),'Status message copied - paste into Zoom, Teams or email');" & linefeed
...
set jsText to jsText & "else if(k==='x')Panels.focus('explain');" & linefeed
...
set jsText to jsText & "else if(k==='m')App.copy(Plain.message(),'Status message copied');" & linefeed
...
set summaryText to "WiFi Connectivity Monitor v3.1 complete."
...
set summaryText to summaryText & return & return & "Tip: open the PLAIN ENGLISH BRIEFING panel (or press X) for a"
set summaryText to summaryText & return & "non-technical explanation and a ready-to-send status message."
set summaryText to summaryText & return & return & "Report folder:" & return & outDir

display dialog summaryText buttons {"OK"} default button "OK" with title "WiFi Connectivity Monitor v3.1"
