...
on run
	...
	display notification "Starting server..." with title appName subtitle "Opening dashboard in browser..."
	
	-- Kill any stuck process on this port from a previous crash
	try
		do shell script "lsof -ti tcp:" & appPort & " | xargs kill -9 2>/dev/null; sleep 0.5" with administrator privileges
	end try
	
	-- Launch the server in background
	-- Use bash -c wrapper to handle complex quoting and ensure cd works
	set launchCmd to "bash -c 'cd " & quoted form of scriptFolder & " && source ~/.zshrc 2>/dev/null; source ~/.bash_profile 2>/dev/null; " & quoted form of pythonBin & " " & quoted form of launcherPath & " > /tmp/mac-maint.log 2>&1 &'"
	do shell script launchCmd
	
	-- Wait up to 6 seconds for server to actually start and respond
	...
	repeat 6 times
		...
		try
			-- Check if port is actually listening (more reliable than curl)
	...
	if serverReady is false then
		...
		set logOutput to do shell script "cat /tmp/mac-maint.log 2>/dev/null || echo 'No log output'"
		display alert "Server Failed to Start" message "The server did not start on port " & appPort & " after " & attempts & " seconds." & return & return & "Log output:" & return & logOutput as critical
		return
	end if
	
	-- Open the browser — server is confirmed running
	do shell script "open http://localhost:" & appPort
